...
public:
  ...
  RandomGen(unsigned int seed_)
      : seed{seed_}, a{1664525}, c{1013904223}, m{(unsigned int)pow(2, 31)} {};
  void set_a(unsigned int a_) { a = a_; };
  void set_c(unsigned int c_) { c = c_; };
  void set_m(unsigned int m_) { m = m_; };
  void reset_seed(unsigned int new_seed_ = (unsigned int)pow(2, 31)) {
    seed = new_seed_;
  };
  double rand();
...
double RandomGen::expo(double mean) { return -log(1 - rand()); }
