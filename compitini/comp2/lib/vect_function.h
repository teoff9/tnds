...
// OSCILLATORE ARMONICO FORZATO 1D
//  x" = -w0^2x - ax' + a0sin(wt)
...
// OSCILLATORE ARMONICO SMORZATO 1D
//  x" = -w0^2x - ax'
class OscSmorzato : public VectFunction<2> {
public:
  OscSmorzato(double w0_, double a_)
      : w0{w0_}, a{a_} {};
  virtual array<double, 2> eval(double t,
                                const array<double, 2> &x) const override {
    array<double, 2> result{x[1], -w0 * w0 * x[0] - a * x[1]};
    return result;
  }
  private:
    double w0, a;
