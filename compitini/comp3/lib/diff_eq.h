...
public:
  virtual array<double, n> step(double, double, array<double, n> &,
                                VectFunction<n> &) const = 0;
};

// EULERO
template <size_t n> class Euler : public DiffEquation<n> {
public:
  virtual array<double, n> step(double t, double h, array<double, n> &x,
                                VectFunction<n> &f) const override {
...
public:
  ...
  array<double, n> evolve_with_prec(double initial_t, double final_t,
                                    double prec, array<double, n> &x,
                                    VectFunction<n> &f, double &wh) const {
    int N{100};
    double h{};
    double xi[2];
    double t{};
    ...
    for (int i{}; i < 2; i++) {
      ...
      h = (final_t - initial_t) /
          N; // arriviamo fino al tempo medio con passo relativo ad N
      x = x_0;
      t = initial_t;
      while (t < final_t - h) {
        x = step(t, h, x, f);
    ...
    // calcola h ottimale
    N = N / 2;
    h = ((final_t - initial_t) / N) *
        pow(15.0 * prec / (16.0 * abs(xi[0] - xi[1])), 0.25);
