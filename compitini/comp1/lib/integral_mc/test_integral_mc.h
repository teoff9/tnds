...
template <typename T> bool is_close____(T n, T m, double delta = 1e-4) {
...
void test_mc_mean() {
  MC_mean m{1};
  MC_mean_multi<2> i{1};
  assert(is_close____(m.integrate(f__, 0, M_PI / 2, 100, M_PI / 2), 1.10443));
  assert(is_close____(i.integrate(g, {1,1}, {2,2}, 100000), 10.2407));
}

void test_mc_hit_miss() {
  MC_hit_miss m{1};
  assert(is_close____(m.integrate(f__, 0, M_PI / 2, 100, M_PI / 2), 0.937612));
}
