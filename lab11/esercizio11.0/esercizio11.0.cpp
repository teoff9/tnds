...
int main() {
    ...
    int N = 10000;
    Gnuplot plt;
    vector<double> th0(N), th1(N), th2(N), dm1(N), dm2(N), n1(N), n2(N), A(N), B(N);
...
double corr(vector<double> &x, vector<double> &y){
    ...
    return 100.*(average(xy)-average(x)*average(y))/(std_dev(x,1)* std_dev(y,1));
}