...
void test_midpoint() {
  ...
  assert(is_close(p.integrate(0, numbers::pi / 2., 10, f),0.9989696941917652));
  assert(is_close(p.integrate(0, numbers::pi / 2., 100, f),0.999989718940119));
  assert(is_close(p.integrate(numbers::pi / 2., 0, 10, f),-0.998969694191765));
  assert(is_close(p.integrate(0, 1, 10, f),0.3005925674684609));
  assert(is_close(p.integrate(1, 2, 30, f),1.440482828731412));
