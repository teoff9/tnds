...
Point3D::Point3D(double new_x, double new_y, double new_z) : x{new_x}, y{new_y}, z{new_z}{}
...
double Point3D::get_phi() const {
    return atan2(y, x); 
}

double Point3D::get_theta() const {
    return acos(z / get_r());
}
double Point3D::get_rho() const {
    return sqrt(x * x + y * y);
}
double Point3D::distance_from(const Point3D &p) const {
    return sqrt(pow(get_x() - p.get_x(), 2) +
              pow(get_y() - p.get_y(), 2) +
              pow(get_z() - p.get_z(), 2));
}