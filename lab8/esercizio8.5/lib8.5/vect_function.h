...
public:
  ...
  virtual array<double, 4> eval(double t,
    ...
    array<double, 4> result{x[2], x[3], -G * M * x[0] / pow(r, 3), -G * M * x[1] / pow(r, 3)};
...
// MOTO DUE CORPI DI CUI UNO MASSIVO FISSO di massa M con CORREZIONE DEL POTENZIALE
// x" = -G * M / r^2 - a/r^3
class CorrectedGrav : public VectFunction<4> {
public:
  CorrectedGrav(double _M, double correction_alpha) : M{_M}, a{correction_alpha} {};
  virtual array<double, 4> eval(double t,
                                const array<double, 4> &x) const override {
